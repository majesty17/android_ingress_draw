...
public class MainActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		HOME_DIR=Environment.getExternalStorageDirectory().getPath();
				//)//getFilesDir().getAbsolutePath();//getExternalStorageDirectory().getAbsolutePath();
		 ...
		 * (Context.WINDOW_SERVICE); //设置window type //wmParams.type =
		 * LayoutParams.TYPE_PHONE; //设置图片格式，效果为背景透明 wmParams.format =
		 * PixelFormat.RGBA_8888; wmParams.flags = //
		 * LayoutParams.FLAG_NOT_TOUCH_MODAL | LayoutParams.FLAG_NOT_FOCUSABLE
		 * // LayoutParams.FLAG_NOT_TOUCHABLE ; //调整悬浮窗口至右侧中间 wmParams.gravity =
		 * Gravity.LEFT | Gravity.TOP; // 以屏幕左上角为原点，设置x、y初始值 wmParams.x = 0;
		 * wmParams.y = 0;
		 * 
		 ...
		 * null); mWindowManager.addView(mFloatLayout, wmParams);
		 * 
		 * //浮动窗口布局视图按钮 Button mFloatView =
		 * (Button)mFloatLayout.findViewById(R.id.float_id);
		 * 
		 * mFloatView.setOnTouchListener(new OnTouchListener() {
		 * 
		 * @Override public boolean onTouch(View v, MotionEvent event) { // TODO
		 * Auto-generated method stub wmParams.x = (int)
		 * event.getRawX()-mFloatLayout.getWidth()/2; wmParams.y = (int)
		 * event.getRawY()-mFloatLayout.getHeight()/2-25;
		 * mWindowManager.updateViewLayout(mFloatLayout, wmParams); return true;
		 * } });
		 */
